import qualified PseudoMap as M
...
setClassName :: String -> String -> Fay ()
setClassName = ffi "document.getElementById(%1).className=%2"

posToString :: Pos -> String
posToString (x,y) = ((show x)++(show y))


setClass :: Pos -> String -> Fay ()
setClass pos str = do
...
setMessage :: String -> Fay ()
setMessage = ffi "document.getElementById('message').innerHTML=%1"


addClickEvent :: String -> Fay () -> Fay ()
addClickEvent = ffi "document.getElementById(%1).addEventListener('click', %2)"
...
getClickEvent board_ref pos = do
  ...
  (b,nd) <- case  p' of
    [] -> return (board,ndisk disk)
    _  -> do
      if (filter (==pos) p')==[] 
        then return (board,disk)
        else do
